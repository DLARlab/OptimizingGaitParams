...
void State_Bounding_CoM::run(){
    ...
    if(_vCmdUser(0) > 25 ){
        _period = 0.55;
        _stancePhaseRatio = 0.6;
        _bias = Vec4(0, 0, 0.65, 0.65);
    }
...
void State_Bounding_CoM::getUserCmd(){
    /* Movement */
    //_vCmdBody(0) =  0.7;//invNormalize(_userValue.ly, _vxLim(0), _vxLim(1), -10, 10);
    _vCmdBody(1) = -invNormalize(_userValue.lx, _vyLim(0), _vyLim(1));
    _vCmdBody(2) = 0;

    // if ( counter < 20){
    //       _vbCmdBodyDhc(0) = counter * 0.01 + 0.05;
    //       _vCmdUser(0) = _vbCmdBodyDhc(0);
    //     }
    // else if ( counter <= 25){
    //     _vbCmdBodyDhc(0) = (counter - 20) * 0.05 + 20 * 0.01 + 0.05;
    //     _vCmdUser(0) = _vbCmdBodyDhc(0);
    // }
    // else if ( counter > 25){
    //   _vbCmdBodyDhc(0) = 0.5;
