...
State_Bounding_SLIP::State_Bounding_SLIP(CtrlComponents *ctrlComp)
             :FSMState(ctrlComp, FSMStateName::BOUNDING_SLIP, "bounding_slip"), 
...
State_Bounding_SLIP::~State_Bounding_SLIP(){
    delete _gait;
}

void State_Bounding_SLIP::enter(){
...
FSMStateName State_Bounding_SLIP::checkChange(){
...
bool State_Bounding_SLIP::checkStepOrNot(){
...
// void State_Bounding_SLIP::setHighCmd(double vx, double vy, double wz){
//     _vbCmdBody(0) = vx;
//     _vbCmdBody(1) = vy;
//     _vbCmdBody(2) = 0; 
//     _dYawCmd = wz;
...
void State_Bounding_SLIP::getUserCmd(){
    /* Movement */
    ...
    if ( counterflight < 20){
          _vbCmdBodyDhc(0) = counterflight * 0.01/2 + 0.05;
          _vCmdUser(0) = _vbCmdBodyDhc(0);
        }
    else if ( counterflight <= 100){
        _vbCmdBodyDhc(0) = (counterflight - 20) *0.008/2 + 20*0.01/2 + 0.05;
        _vCmdUser(0) = _vbCmdBodyDhc(0);
    }
    else if ( counterflight > 150){
      _vbCmdBodyDhc(0) = (counterflight - 100) * 0.006/2 + 80*0.008/2 + 20 * 0.01/2 + 0.05;
      _vCmdUser(0) = _vbCmdBodyDhc(0);
    }
    else{
        _vbCmdBodyDhc(0) = 0.6;
    ...
    // else if ( counterflight > 500){
    //   _vbCmdBodyDhc(0) = 3.845 - (counterflight - 500) * 0.015;
    //   _vbCmdBodyDhc(0) = saturation(_vbCmdBodyDhc(0), Vec2(0.0,  3.845));
    //   _vCmdUser(0) = _vbCmdBodyDhc(0);
    // }
...
void State_Bounding_SLIP::calcCmd(){
    /* Movement */
    ...
    if((*_contact)(0) == 1 || (*_contact)(1) == 1){
        ...
        _wfhcd(2) = -sin ((M_PI/stanceIteration) * counterFrontUse) * (0.01 + _vbCmdBodyDhc(0) * 0.0025) + (0.3 - _vbCmdBodyDhc(0) * 0.01);//0.28 - _paramBound;//
        ...
        _wrhcd(2) = -sin ((M_PI/stanceIteration) * counterRearUse) * (0.01 + _vbCmdBodyDhc(0) * 0.0025) + (0.3 - _vbCmdBodyDhc(0) * 0.01);
 ...
 */
void State_Bounding_SLIP::calcTau(){
    // if((*_contact)(0) == 1 || (*_contact)(1) == 1){
...
////////////////////////////////////////////////////////
    ...
    if (counter > 2000){
        if((*_contact)(0) == 1 || (*_contact)(1) == 1){
            ...
            if((*_phase)(1) > 0.39){
                for (int i = 0; i < 2; i++){
                    _forceFeetGlobalAdd[i] =  0 * (_wPosFhc(2) - _wfhcd(2))+ 1.0 * (_VelFhc(2) - 0);
                }
                _Kpp = Vec3(150, 70, 200).asDiagonal();
                _Kdp = Vec3(20, 20, 30).asDiagonal();
            ...
            if((*_phase)(2) > 0.39){
                for (int i = 2; i < 4; i++){
                    _forceFeetGlobalAdd[i] =  0* (_wPosRhc(2) - _wrhcd(2))+ 1.0 * (_VelRhc(2) - 0);
                }
                _Kpp = Vec3(70, 70, 70).asDiagonal();
                _Kdp = Vec3(20, 20, 20).asDiagonal();
                _kpw = Vec3(400, 800, 400).asDiagonal();
 ...
 * 
 */
void State_Bounding_SLIP::calcQQd(){
...
Vec3 State_Bounding_SLIP::xProd(Vec3 vec1, Vec3 vec2){
    Vec3 vec3;
...
void State_Bounding_SLIP::plotData(std::vector<std::vector<double>>& desd_data, std::vector<std::vector<double>>& actual_data){
...
void State_Bounding_SLIP::plotSubPlot(std::vector<std::vector<double>>& desd_data, std::vector<std::vector<double>>& actual_data, int legNum){
...
void State_Bounding_SLIP::plotGRF(std::vector<std::vector<double>>& desd_data, std::vector<std::vector<double>>& actual_data){
...
void State_Bounding_SLIP::plotXZ(std::vector<std::vector<double>>& desd_data, std::vector<std::vector<double>>& actual_data){
