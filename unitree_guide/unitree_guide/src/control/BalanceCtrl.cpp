...
BalanceCtrl::BalanceCtrl(double mass, Mat3 Ib, Mat6 S, double alpha, double beta)
    ...
    _fricMatRear <<  1,  0, _fricRatioRear,
                    -1,  0, _fricRatioRear,
                     0,  1, _fricRatioRear,
                     0, -1, _fricRatioRear,
                     0,  0, 1;   
...
BalanceCtrl::BalanceCtrl(QuadrupedRobot *robModel){
    ...
    _alpha = 0.001;
    _beta  = 0.1;
    _fricRatio = 0.4;  ///0.4 original, 0.6 for RAL

    //2. use samll fricRatio for rear legs
    _fricRatioRear = 0.6;  ///for Rear legs
    // _fricRatioRear = 0.4;  ///for Rear legs
...
void BalanceCtrl::calConstraints(VecInt4 contact){
    ...
    for(int i(2); i<4; ++i){
        if(contact(i) == 1){
            _CI.block(5*ciID, 3*i, 5, 3) = _fricMatRear;
            ++ciID;
        }else{
            _CE.block(3*ceID, 3*i, 3, 3) = I3;
            ++ceID;
